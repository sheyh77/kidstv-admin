...
function MultCard() {
  ...
  // Ma'lumotlarni olish
  useEffect(() => {
    fetch("https://ceed8a646c7fba8b.mokky.dev/brainmult")
      .then((res) => res.json())
  ...
  async function handleSave() {
    ...
    if (editingData) {
      ...
      try {
        const res = await fetch(`https://ceed8a646c7fba8b.mokky.dev/brainmult/${editingData.id}`, {
          method: "PUT",
      ...
      try {
        const res = await fetch("https://ceed8a646c7fba8b.mokky.dev/brainmult", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify(values),
        });
  ...
  async function handleDelete(id) {
    try {
      await fetch(`https://ceed8a646c7fba8b.mokky.dev/brainmult/${id}`, {
        method: "DELETE",
      });
  ...
  return (
    <section className="brain">
      <div className="brain-add">
        <h1 className="brain-add-title">Bilimdon categoria qo'shish</h1>
        <button className="brain-add-btn" onClick={AddMenuOpen}>+ Add</button>
      </div>
      {/* Modal */}
